...
bias_gyro_x = 0.07529136114650911 # [rad/measurement]
bias_gyro_y = 0.0417171761262585 # [rad/measurement]
bias_gyro_z = 0.0008089498350258776 # [rad/measurement]

# variances (Based on analysis on the static measurements)
gyro_x_var = 0.09433*pi / 180
pitchVar = 0.38659*pi / 180
...
for line in f:
    ...
    pitch = pitch = atan2(acc_y, sqrt(acc_x**2+acc_z**2))
    roll = roll = atan2(-acc_x, acc_z)
    ...
    gyroVarAcc += gyro_x_var
    est_pitch = estAngle + gyro_x*(ts_now-ts_prev)
    estVar_prev = estVar + gyroVarAcc*(ts_now-ts_prev)

    # Kalman correction step (we have new data in each iteration)
    k_gain = estVar_prev / (estVar_prev+pitchVar)
    est_pitch += k_gain * (pitch-est_pitch)
    estVar = estVar_prev * (1-k_gain)
    estAngle = est_pitch
    gyroVarAcc = 0

    # define which value to plot as the Kalman filter estimate
    kalman_estimate = est_pitch
...
if showPlot == True:
    ...
    plt.ylim(-60, 10)
    plt.savefig('./results/imu_exercise_acc_kalman.png')
    plt.draw()
